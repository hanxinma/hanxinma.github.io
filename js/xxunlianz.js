...
var link3Element = document.querySelector(".link3");
if (link3Element) {
        link3Element.addEventListener("click", function () {
                根队列 = [...根队列3];
                显示当前根();
                显示所有根();
        });
...
function showPopup() {
        setTimeout(() => {
                alert('默认模式训练需要读出音，或电脑端优先，听语音记忆，因为拼音打字以想音为主\n【尽量不要看拼音记】因为会打字的时候想不起来\n语音读的是汉字所以会不准，可以按回车让他再读，或按Ctrl显示文字\n韵母组单词法记忆这个练熟了，就可以打字了。\n但是打字会难受,很慢.坚持一个星期就会过去\n或者先不打字,练习后面单个韵母条反,练熟,上手打字就不用想.\n或练习,前500单字,把这些练熟了再打字,就不会有痛苦期.\n手机端,无语音,手机端需刷新保存进度\n[注意],点链接会导致进度丢失');
        }, 1000);
}
showPopup();
...
// 首先尝试获取页面上的目标元素
var linkElement = document.querySelector('.link1');
...
document.addEventListener('DOMContentLoaded', function () {
        var link0Element = document.querySelector('.link1');
...
if (自动模式 === "开") {
        听写模式 = "关"; //这里 配合 听写模式和秒先模式 开关用，默认给关了
}
else {
        听写模式 = "开";
        //两个有用，需要这个变量
...
document.getElementById("秒显按钮").addEventListener("click", function () {
        if (秒显模式 === "关") {
                秒显模式 = "开";
        ...
        // 更新按钮的文本内容
        var 按钮 = document.getElementById("秒显按钮");
        按钮.innerText = 秒显模式 === "关" ? "已关秒显" : "已开秒显";
});
...
document.getElementById("语音模式").addEventListener("click", function () {
        语音模式 = !语音模式; // 切换语音模式的状态
        ...
        if (语音模式) {
                ...
                alert("注意！消失后按CTRL键显示题目，按回车键再次语音，按空格学习答案---优先推荐这个模式，这个模式更适合音码"); // 弹出提示框显示消息
        } else {
                console.log("语音模式不好"); // 关闭语音模式时输出 "不好"
                秒显模式 = "关"
        ...
        // 更新按钮的文本内容
        var 按钮 = document.getElementById("语音模式");
        按钮.innerText = 语音模式 ? "已开语音模式" : "已关语音模式";
});

let 秒显模式 = "关" //改了这里为关 ，这里默认给关了。
...
function 显示当前根() {
        ...
        if (听写模式 === "关") {
                ...
                else { 当前根显示.innerHTML = 根队列[当前索引].split("\t")[0]; }
...
function 检查答案() {
        ...
        console.log("判断答案长度的值----------", 判断答案长度的值);


        let 错误消息 = document.getElementById("错误消息");
        console.log("正确答案----------", 正确答案);
        console.log("根队列整个----------", 根队列整个);
...
function 连击加分函数() {
        ...
        else if (答对分数 >= 50 && 答对分数 < 100) {
                答对总分数 += 答对分数 * 20;
                加分值 = 答对分数 * 20;
        }
        else if (答对分数 >= 100 && 答对分数 < 1000000) {
                答对总分数 += 答对分数 * 30;
                加分值 = 答对分数 * 30;
        }
        return { 答对总分数, 加分值 };
...
function 错误显示消失函数(correctAnswer) {
        ...
        错误消息.innerHTML = correctAnswer + " <span style='color: gray;'>" + 第四列 + "</span>";
...
function 显示当前根无动画() {
        ...
        setTimeout(function () {
                当前根显示.innerHTML = '🐉读音'; // 清空当前根显示内容
        }, 条反等待时间 - 1000); // 3000毫秒 = 3秒
}


var 输入框 = document.getElementById('用户输入'); //等待几秒后用户是还不输入，判断要不要显示条反
var 定时器ID = null;

function 条反判断() {
        clearTimeout(定时器ID);

        定时器ID = setTimeout(function () {
                if (输入框.value === '') {
                        检查答案();
                        显示当前根无动画();
                }
...
let 条反等待时间 = 3000;
document.addEventListener("DOMContentLoaded", function () {
        const 条反模式 = document.getElementById("条反模式");
        // 条反模式.click();  // 给条反默认打开
        // 条反模式.click(); // 在给条反默认关闭
});
...
let 条反开关 = "关";
条反模式.addEventListener("click", function () {
        if (条反开关 === "开") {
                条反开关 = "关";
                条反模式.innerText = '条反模式-已关';
        } else {
                条反开关 = "开";
                条反模式.innerText = '条反模式-已开';
        }
...
function 条反时间自定义() {
        let userInput = prompt("请输入条反等待的时间，123456任意一个数字，单位秒");
